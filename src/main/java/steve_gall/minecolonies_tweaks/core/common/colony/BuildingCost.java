...
public record BuildingCost(BlockPos id, List<ItemStorage> costs)
{
	...
	public static BuildingCost deserialize(HolderLookup.Provider provider, CompoundTag tag)
	{
		...
		var costs = NBTUtils.streamCompound(tag.getList(TAG_COSTS, Tag.TAG_COMPOUND)).map(SerializationHelper.<ItemStorage> deserializerTag(provider)).toList();
	...
	public static CompoundTag serialize(HolderLookup.Provider provider, BuildingCost data)
	{
		...
		var costsTag = data.costs.stream().map(SerializationHelper.serializerTag(provider)).collect(NBTUtils.toListNBT());
