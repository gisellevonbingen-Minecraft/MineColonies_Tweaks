	...
	public static Map<EventType, List<Tuple<SoundEvent, SoundEvent>>> registerJobSoundEvents(DeferredRegister<SoundEvent> register, String name)
	{
		...
		for (var event : EventType.values())
		{
			...
			for (var i = 1; i <= 4; i++)
			{
				...
				var maleSoundEvent = new SoundEvent(new ResourceLocation(namespace, prefix + ".male" + suffix));
				var femaleSoundEvent = new SoundEvent(new ResourceLocation(namespace, prefix + ".female" + suffix));
